...
export default function TheadRow({
    name,
    sortUsers,
    unsortUsers
}) {
    let [showSvg, setShowSvg] = useState(false);
    function onShowSvg(event) {
        if (!event.target.children[0]) {
            setShowSvg(true);
            sortUsers(event.target);
        } else {
            setShowSvg(false);
            unsortUsers(event.target);
        }
    }
    return (
        <th onClick={onShowSvg}>
            {name}
            {showSvg
